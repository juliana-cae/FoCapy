...
export function taskHistoryForFilter(sessions=[],taskId=''){const key=String(taskId||'');return (Array.isArray(sessions)?sessions:[]).filter(session=>session?.status==='completed'&&Array.isArray(session.taskSnapshots)&&(!key||session.taskSnapshots.some(task=>task.id===key)));}
