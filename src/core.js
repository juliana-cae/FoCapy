...
export function taskStatisticsForPeriod(tasks = [], period = 'daily', now = new Date()) {
  const definition=seriesDefinition(period,now),priorities=['low','normal','high','urgent'].map(priority=>({priority,completed:0,open:0})),priorityMap=new Map(priorities.map(item=>[item.priority,item])),categoryMap=new Map();let completedTotal=0,openTotal=0,subtasksTotal=0,subtasksDone=0;
