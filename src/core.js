...
export function normalizeTasks(tasks = []) { return (Array.isArray(tasks) ? tasks : []).reduce((result, task, index) => { const title = String(task?.title ?? task ?? '').trim().replace(/\s+/g, ' ').slice(0, 100); if (!title) return result; const normalized={ id: String(task?.id || `task-${index + 1}`), title, done: Boolean(task?.done), ...(task?.completedAt?{completedAt:String(task.completedAt)}:{}) }; if(String(task?.tag||'').trim()){normalized.tag=String(task.tag).trim().slice(0,32);normalized.tagColor=normalizeTagColor(task?.tagColor);normalized.tagFontColor=normalizeTagColor(task?.tagFontColor,DEFAULT_TAG_FONT_COLOR)} if(task?.categoryId)normalized.categoryId=String(task.categoryId); if(Number.isFinite(Number(task?.order)))normalized.order=Number(task.order); if(['low','normal','high','urgent'].includes(task?.priority))normalized.priority=task.priority; result.push(normalized); return result; }, []); }
export function addTask(tasks = [], title = '', tag = '', tagColor = DEFAULT_TAG_COLOR, tagFontColor = DEFAULT_TAG_FONT_COLOR, priority = DEFAULT_TASK_PRIORITY, categoryId = '') { const normalized = normalizeTasks(tasks); return normalizeTasks([...normalized, { id: `task-${Date.now()}-${normalized.length + 1}`, title, tag, tagColor, tagFontColor, priority, categoryId, order: normalized.length, done: false }]); }
...
export const TASK_PRIORITIES = { low: { label: 'Baixa', color: '#c9e8bf' }, normal: { label: 'Normal', color: '#c7e1f5' }, high: { label: 'Alta', color: '#f6c27a' }, urgent: { label: 'Urgente', color: '#ef9696' } };
