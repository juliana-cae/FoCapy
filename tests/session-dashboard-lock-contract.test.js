...
test('only a running Blindada session locks the focus dashboard; pause restores app navigation',()=>{
  assert.match(app,/const active=state\.current\?\.status==='running',pomoPaused=state\.current\?\.status==='paused'&&Boolean\(state\.current\?\.pomoPhase\),locked=active&&state\.strict;document\.documentElement\.classList\.toggle\('session-active',active\);document\.documentElement\.classList\.toggle\('session-active-lock',locked\)/);
  assert.doesNotMatch(app,/window\.scrollTo\(0,168\)/);
